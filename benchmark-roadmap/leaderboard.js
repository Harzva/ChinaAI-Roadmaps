...
function renderMaintenance() {
  ...
  const values = [
    ...
    [item.queue.paperEvidence, 'paper evidence'],
...
function renderRegistry() {
  ...
  $('#registry-body').innerHTML = rows.map((item) => {
    ...
    const stageLabel = item.evidenceStage === 'implementation_index' ? '实现目录已核验' : item.evidenceStage === 'paper_evidence' ? '论文证据待升格' : '原始来源已建档';
...
async function openBenchmark(id) {
  ...
  const evidenceLabel = registryEntry?.evidenceStage === 'implementation_index' ? '实现目录已核验 · 原始来源待审核' : registryEntry?.evidenceStage === 'paper_evidence' ? 'arXiv 论文身份已核验 · 成绩仍在隔离审核' : '原始来源已建档';
